...
- **`sft.py`**: `run_rollout_eval` returns `RolloutEval` with `overall`/`overall_eot` and per-`LessonKind` breakdowns; checkpoint is selected on the EOT-respecting `eval/thput`. PPO reuses this for the same `eval/` keys (lazy import to avoid the ppo↔sft cycle).
...
- **`eval/`** — periodic EOT-respecting greedy held-out throughput (`eval/thput`, `eval/{LESSON}/thput`), every `--eval-every` iters. SFT logs the same keys from the same `run_rollout_eval`, so the two are literally overlay-able on one panel (the eval *sets* still differ: SFT validates on seeds from `args.seed` up, PPO on `_build_eval_set`'s 9,000,000+ range). **This is the headline progress signal**, and the sweep metric (`ci/sweep_ppo.yaml`).
- **`rollout/`** — on-policy sampled episode stats (`thput`, `thput_raw`, `reward`, `length`, `eot_rate`, `invalid_frac`, `num_entities`, `entity_efficiency`, `frac_reachable`, `entity_cost`) + per-lesson `rollout/{LESSON}/{thput,reward,length,entity_cost}`. `cost_efficiency` is not logged — it is `1/(1 + entity_cost_scale * entity_cost)`, pinned down by `entity_cost` and `reward`. `thput_raw` is overall-only — it is the reward's input, but *within* a lesson the ceiling is fixed so raw is just an affine rescaling of `thput` (measured within-run correlation 1.000 on every fixed-ceiling lesson).
...
- **`losses/`** `policy,value,entropy,total,approx_kl,clipfrac` (explained variance lives in `critic/`, the richer view); **`optim/`** `lr,critic_lr,ent_coef,grad_norm,critic_warmup`; **`perf/`** `sps,rollout_seconds,update_seconds,eval_seconds` (the only eval-timing key — there is no `eval/seconds`).
...
**The RL goal is for PPO's achieved throughput to exceed the SFT base's `eval/thput` (≈0.11 for `j0s5y2mc`).** Per-lesson the SFT base is uneven (MOVE_ONE_ITEM ~0.38, assembler lessons ~0).
