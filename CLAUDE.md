...
- `scripts/model_gallery.py` — Generates a **static, self-contained HTML site** (index + one page per lesson) of a checkpoint's greedy factory completions — each factory shown as problem / model build / ground truth with achieved throughput. Inference runs once at generation time; the output is plain HTML (icons deduped into CSS), so it can be hosted on GitHub Pages (`.github/workflows/model-gallery.yml` runs the generator and deploys). Reuses `_resolve_wandb_checkpoint`, `FactorioEnv`, `AgentCNN`, and `world2html`.
