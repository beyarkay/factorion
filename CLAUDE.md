...
2. **SFT pretraining** — `sft.py` runs supervised training on those pairs, giving the policy a strong prior over entity placement. The `MEMORISE_*` lessons are held out of the training stream (`sft.TRAIN_EXCLUDED_KINDS`) but stay in val, so `val/MEMORISE_*/thput` reads transfer from the other lessons rather than imitation. It uploads the best checkpoint as a W&B artifact (type `model`, containing `sft_checkpoint.pt`) so RL can pull it by run id.
