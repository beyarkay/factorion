...
- **`ppo.py`**: `PpoArgs` (all PPO hyperparams + `start_from`, `critic_warmup`, `eval_every`; defined in `training_config.py`), `FactorioEnv` (`reset`/`step`; terminal reward = `thput_raw / (1 + entity_cost_scale * entity_cost)`, where entity cost is per-output recursively-expanded raw-item quantity + craft time; normalized throughput remains a reference-based evaluation diagnostic and is not used by the reward; `eot` is a real action that ends the episode; `info['kind']` carries the lesson for per-lesson logging), `AgentCNN` (encoder + per-head outputs: tile/entity/direction/item/misc + `critic_head` + `eot_head`; stashes `_last_head_entropy`/`_last_eot_prob` for the policy/* metrics; the entropy bonus is a per-head weighted sum — `ent_mult_*`, optionally divided by `max_head_entropies()` under `--ent-normalize` — with the EOT head at **0** so exploration pressure can't drag termination toward p=0.5), `_resolve_start_from`/`_resolve_wandb_checkpoint` (checkpoint loading), `_run_signature` (run name), `_build_eval_set`/`_run_greedy_eval` (the eval/ section).
...
- **`eval/`** — periodic EOT-respecting greedy held-out throughput (`eval/thput`, `eval/{LESSON}/thput`), every `--eval-every` iters; directly overlay-able with the SFT baseline. **This is the headline progress signal**, and the sweep metric (`ci/sweep_ppo.yaml`). Alongside it, on the same rollout: `eval/solve_rate` (fraction of held-out factories actually *finished*, i.e. `thput_normed >= 1`, which a mean throughput cannot distinguish from half-building everything) and `eval/eot_prob` (the EOT head's mean probability), both with `trial_`/per-lesson splits.
- **`rollout/`** — on-policy sampled episode stats (`thput`, `thput_raw`, `reward`, `length`, `eot_rate`, `invalid_frac`, `num_entities`, `entity_efficiency`, `frac_reachable`, `entity_cost`, `cost_efficiency`) + per-lesson `rollout/{LESSON}/{thput,thput_raw,reward,length,eot_rate,entity_cost,cost_efficiency}` (raw items/s kept alongside the normalized throughput so lessons with very different ceilings stay comparable). Episode length also gets a shape, not just a mean — `length_{p10,p50,p90,max,le2_frac}` — since a mean cannot tell instant-quitting apart from steady building.
...
- **`policy/`** — acting-policy distribution: `entropy`, per-head `entropy_{tile,entity,direction,item,misc,eot}`, `eot_prob`. The raw per-head entropies are not comparable to each other (the tile head's max is ~4.8 nats, misc's ~1.1), so `entropy_weighted{,_head}` reports each head's actual pull on the loss, `ent_coef * ent_mult_{head} * H_head`.
