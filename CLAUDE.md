...
- **State tensor** is `(C, W, H)` with `C = len(Channel) = 6` channels: `ENTITIES`, `DIRECTION`, `ITEMS` (recipe/filter), `MISC` (underground up/down), `FOOTPRINT` (1 = buildable), `ORES` (Item id of the ore terrain on the tile, 0 = none; buildable-over, never blanked).
- **Lessons** (`LessonKind` in `factorion.py`, built from `factory_gen.rs::LessonKind` via `factorion_rs.py_lesson_kinds()`): `MOVE_ONE_ITEM`, `SPLITTER_SPLIT`, `SPLITTER_MERGE`, `ASSEMBLE_1IN_1OUT`, `MOVE_VIA_UG_BELT`, `ASSEMBLE_2IN_1OUT`, `MEMORISE_1_INGREDIENT_RECIPES` … `MEMORISE_4_INGREDIENT_RECIPES`, `MOVE_ONE_ITEM_CHAOS`, `CROSS_UNDER_BELT`, `FACTORY_1_INGREDIENT`, `MINE_ORE`. `build_factory(size, kind, seed, ...)` (Rust-backed) returns `Optional[Factory]` (rejection sampling can fail → `None`); `blank_entities(factory, num_missing_entities, seed)` removes N entity *units* (multi-tile entities count as one). Each lesson has a `build_*` generator in `factory_gen.rs`; the `MEMORISE_N_INGREDIENT_RECIPES` lessons (`build_memorise_recipes`, one per exact ingredient count 1–4) place a random recipe's assembler fed/drained by `source → belt → inserter → assembler → inserter → belt → sink` arms with exactly one belt per arm (one source per ingredient, so lesson `N` has exactly `N` input sources); `CROSS_UNDER_BELT` (`build_cross_under_belt`) is validated by its own throughput/orphan invariants; `FACTORY_1_INGREDIENT` (`build_factory_1_ingredient`) places a row of 1+ assemblers sharing an input and an output belt lane (1–3 input and 1–3 output inserters per assembler), with the source/sink at semi-arbitrary cells wired to the lane ends by the UG-aware router — throughput deliberately varies (input-inserter / recipe-speed / output-inserter limited) so the critic sees good and bad layouts; `MINE_ORE` (`build_mine_ore`) puts a 3×3 electric mining drill on a random single-type ore patch (`ORES` channel) belt-routed to a sink.
- **Mining drills**: `ElectricMiningDrill` is 3×3, directional, and mines the `ORES` terrain under its 5×5 area (body +1 each side; drills may overlap patches) at a flat 0.5 items/s whenever ≥1 ore tile is covered (split across ore types by covered tile count on mixed patches — coverage gates operation, it does not scale speed). Output goes to exactly ONE tile — one step beyond the middle of the facing edge — and only onto belt-like receivers (belt/UG/splitter/sink-as-chest; inserters can never pull from a drill), landing on one lane by the inserter far/right rule. Engine-wise it is a finite-rate flow root: output pre-seeded in `graph.rs` (`ElectricMiningDrill::mining_output`), BFS root in `throughput.rs` alongside `Source`. Ore types: copper/iron ore, coal, stone (`Item::is_ore`).
...
- **No-orphan invariant**: no lesson's solved factory may contain orphan tiles — throughput must report `unreachable == 0` (checked by `factory_gen.rs::tests::test_no_orphan_tiles_every_lesson` across every `LessonKind`). Unreachability is judged per ENTITY (an entity is an orphan iff none of its lane nodes lie on a source→sink path) since e.g. an inserter-fed belt legitimately has one forever-empty lane.
...
- **Rendering factories to ASCII** — when asked to "render"/"show"/"draw" factories, use the real renderer, never hand-roll one: `from factorion import render_factory; print(render_factory(factory))` (accepts a `Factory` or a `(C, W, H)` world tensor; Rust binding `factorion_rs.render_factory(world_WHC)` lives in `factorion_rs/src/render.rs`). It returns the two-char grid format the textual fixtures use — `b`=belt, `i`=inserter, `l`=long-handed inserter, `a`=assembler box, `Y`=splitter, `m`=mining-drill box (facing shown as one marker on its fore-edge-center tile), `d`/`u`=underground down/up, `S`=source, `K`=sink, each plus a facing marker `^>v<`; `..` is an empty tile (ore terrain is not drawn — it lives in the `ORES` channel / fixture `ores:` blocks). To also show each factory's recipe, read the assembler's tagged item from the `ITEMS` channel (or the sink's carried item).
