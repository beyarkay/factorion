...
- **`trial_thput`** (`trial_overall_eot`) — the same number over trial kinds only, pooled separately (`trial_n` counts the trials scored). Zero when no trial ran, which is always the case for SFT.
