...
- **`ppo.py`**: `PpoArgs` (all PPO hyperparams + `start_from`, `critic_warmup`; defined in `training_config.py`), `FactorioEnv` (`reset`/`step`; terminal reward = `thput_raw / (1 + entity_cost_scale * entity_cost)`, where entity cost is per-output recursively-expanded raw-item quantity + craft time; normalized throughput remains a reference-based evaluation diagnostic and is not used by the reward; `eot` is a real action that ends the episode; `info['kind']` carries the lesson for per-lesson logging), `AgentCNN` (encoder + per-head outputs: tile/entity/direction/item/misc + `critic_head` + `eot_head`; stashes `_last_head_entropy`/`_last_eot_prob` for the entropy/* metrics), `_resolve_start_from`/`_resolve_wandb_checkpoint` (checkpoint loading), `_run_signature` (run name), `_rollout_episode_metrics` (the sampled/ section).
- **`sft.py`**: `run_rollout_eval` returns `RolloutEval` with `overall`/`overall_eot` and per-`LessonKind` breakdowns; checkpoint is selected on the EOT-respecting `greedy/thput`, flattened into the logged keys by `_greedy_metrics`. PPO does not call it — it reports only `sampled/`.

## W&B dashboards

Runs are named by a hyperparameter signature, not a timestamp (`ppo.py:_run_signature`, `sft.py:_artifact_name`), e.g. `ppo-s11-lr5e-05-ent0-cw10-fromj0s5y2mc-c93-69-96-seed1`. `global_step` (env steps) is the PPO x-axis. PPO logs once per iteration (see the `define_metric` block in `ppo.py`), SFT once per eval window, into these sections:

**A metric's prefix names how the number was measured, and nothing else.** `sampled/` = PPO's on-policy sampled episodes; `greedy/` = SFT's argmax replay from a blank grid; `val/` = SFT's teacher-forced next-action scoring. The same key under two prefixes is the same formula measured two ways; a name appears under one prefix only. PPO logs `sampled/`, SFT logs `greedy/` + `val/` — **PPO does not measure greedy play and SFT does not measure sampled play** (each cost real time to reproduce a number the other prefix already tracked: PPO's deleted greedy eval agreed with `sampled/thput` within 2% across five finished runs, in both directions).

- **`sampled/`** (PPO) — on-policy sampled episode stats (`thput`, `thput_raw`, `reward`, `length`, `eot_rate`, `eot_prob`, `invalid_frac`, `num_entities`, `entity_efficiency`, `frac_reachable`, `entity_cost`) + per-lesson `sampled/{LESSON}/{thput,reward,length,entity_cost}`. **This is PPO's headline progress signal**, and its sweep metric (`ci/sweep_ppo.yaml`). Every episode builds a fresh never-repeated factory, so this is already a held-out generalization measure, and `eot` terminating the episode means it already respects the stop head. `cost_efficiency` is not logged — it is `1/(1 + entity_cost_scale * entity_cost)`, pinned down by `entity_cost` and `reward`. `thput_raw` is overall-only — it is the reward's input, but *within* a lesson the ceiling is fixed so raw is just an affine rescaling of `thput` (measured within-run correlation 1.000 on every fixed-ceiling lesson).
- **`greedy/`** (SFT) — EOT-respecting greedy held-out rollout, assembled by `sft.py:_greedy_metrics`: `thput`, `asm_item_acc` (recipe-pick), `eot_acc` / `eot_pos_recall` (does it stop when the factory is actually done?) + per-lesson each. **SFT's headline signal** and its sweep metric (`ci/sweep_sft.yaml`); the checkpoint is selected on `greedy/thput`. Per-lesson keys only appear for lessons actually scored (no assembler → no `asm_item_acc`).
- **`entropy/`** (PPO) — acting-policy distribution: `total` + per-head `{tile,entity,direction,item,misc,eot}`. The entropy *bonus* is `optim/ent_coef × entropy/total`; it is not logged separately.
...
- **`losses/`** — what the optimiser minimises. PPO: `pg` (clipped surrogate policy-gradient loss), `value`, `total`, `approx_kl`, `clipfrac` (explained variance lives in `critic/`, the richer view). SFT's loss decomposition stays under `val/loss*`, measured on the fixed val set.
- **`optim/`** — PPO `lr,critic_lr,ent_coef,grad_norm,critic_warmup`; SFT `lr,grad_norm`.
- **`perf/`** — PPO `sps,rollout_seconds,update_seconds`; SFT `train_seconds,train_data_seconds,train_compute_seconds,val_seconds,eval_seconds` (`perf/eval_seconds` times the greedy rollout — there is no `greedy/seconds`).
- **`val/`** (SFT) — teacher-forced next-action scoring on a fixed 400-factory held-out set: `loss`, `loss_{tile,ent,dir,item,misc,eot}`, `acc`, `{tile,ent,dir,item,misc}_acc`, `eot_acc`, `eot_pos_recall`, `not_none_{ent,dir,item,misc,eot}_acc` + per-lesson. Distinct from `greedy/eot_acc`: `val/eot_acc` scores the stop head against expert labels, `greedy/eot_acc` against the model's own rollout. There is no `train/` namespace — at the default `--epochs 1` the stream never repeats a factory, so training-batch metrics estimated the same quantity as `val/`. (Caveat: `--epochs > 1` and `--dataset-cache` *do* replay factories, and a genuine train-vs-val gap is no longer visible.)
- **Top-level** (SFT) — `epoch`, `samples_seen`, `global_step`: x-axes, not measurements.
...
**The RL goal is for PPO's `sampled/thput` to exceed the SFT base's `greedy/thput` (≈0.11 for `j0s5y2mc`).** Per-lesson the SFT base is uneven (MOVE_ONE_ITEM ~0.38, assembler lessons ~0). That bar compares across prefixes on purpose: both use the same `thput_normed` formula over the same factory population, and only action selection differs — worth ≤2% with no consistent direction, measured across five finished PPO runs when both numbers were still logged side by side.
...
assert pr:sampled/thput > main:sampled/thput
/ci sweep sft --pods 2
