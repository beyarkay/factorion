...
- **Lessons** (`LessonKind` in `factorion.py`, built from `factory_gen.rs::LessonKind` via `factorion_rs.py_lesson_kinds()`): `MOVE_ONE_ITEM`, `SPLITTER_SPLIT`, `SPLITTER_MERGE`, `ASSEMBLE_1IN_1OUT`, `MOVE_VIA_UG_BELT`, `ASSEMBLE_2IN_1OUT`, `MEMORISE_1_INGREDIENT_RECIPES` … `MEMORISE_5_INGREDIENT_RECIPES`, `MOVE_ONE_ITEM_CHAOS`, `CROSS_UNDER_BELT`. `build_factory(size, kind, seed, ...)` (Rust-backed) returns `Optional[Factory]` (rejection sampling can fail → `None`); `blank_entities(factory, num_missing_entities, seed)` removes N entity *units* (multi-tile entities count as one). Each lesson has a `build_*` generator in `factory_gen.rs`; the `MEMORISE_N_INGREDIENT_RECIPES` lessons (`build_memorise_recipes`, one per exact ingredient count 1–5) place a random recipe's assembler fed/drained by `source → belt → inserter → assembler → inserter → belt → sink` arms with exactly one belt per arm (one source per ingredient, so lesson `N` has exactly `N` input sources); `CROSS_UNDER_BELT` (`build_cross_under_belt`) is validated by its own throughput/orphan invariants.
...
- **Dual-lane belts** (`wiki/two-lane-system.md`, #65): graph nodes are NOT 1:1 with entities — belt-ish tiles (belt, UG, each splitter tile) emit one node per `Lane` (`NodeId.lane`, labels `name:L\n@x,y`), capped at `LANE_FLOW_RATE = 7.5` i/s each; inserter/assembler/source/sink stay single-node. Straight/curve/tunnel/splitter flow is lane-preserving (`entities.rs::lane_preserving_edges`); a perpendicular feed onto a belt with any other belt-connectable input sideloads onto the near-side lane (`feed_edges`/`belt_feeders`/`is_curved_belt` — lone side feed = curve; UG mouths and splitters never curve); inserters drop onto exactly one lane (far when perpendicular, belt's right when in-line) and pick up with lane priority (near when perpendicular, left when in-line/curved) via greedy fill in `throughput.rs::pickup_input`.
