...
`ppo.py --start-from <ckpt>` loads the full SFT `AgentCNN` state dict (encoder + every policy/eot head). The **critic head is the only part SFT never trained**, so `--critic-warmup N` freezes the actor (encoder + policy heads) for N iterations and trains the value head alone against the fixed SFT features before joint PPO begins; LR envelopes + entropy schedule restart at the unfreeze. For finetuning an SFT policy, set `--ent-coef-start/--ent-coef-end 0` and a small `--learning-rate` (e.g. 5e-5); `--divergence-penalty β` additionally anchors the actor to the loaded checkpoint with a `β·KL(π_θ ‖ π_ref)` loss penalty against a frozen copy (#237), guarding SFT skills on lessons the current reward isn't exercising.
