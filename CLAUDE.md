...
- **Observation tensor** is `(OBS_CHANNELS, W, H)` — the world's channels plus a sixth, derived `FLOW` channel at index `CH_FLOW`: the steady-state items/s the engine computes through each entity, painted across its whole footprint, in `FLOW_SCALE`(=8) eighths of an item/s and clamped to `MAX_ENTITY_FLOW`(=30, a saturated splitter). It is **not** a world channel — nothing places it, it is stale the moment a tile changes, and `Factory.world_CWH` never carries it. Build one with `factorion.update_with_flow_and_thput(world_CWH) -> (obs, thput, num_unreachable)`, the single place the network's input is assembled (env, SFT demos, the builder UI, the mod server) — it returns the throughput and unreachable count alongside because `simulate_throughput` computes all three in one pass. An entity carries items as soon as something feeds it — reaching a sink is not required, so a chain built out from a source lights up tile by tile while throughput is still flat zero (delivery stays legible at the sink, whose tile reads its own rate). A belt loop reads zero throughout, which is the one failure `unreachable` cannot see (it reports 0). The channel is integer-valued so SFT's uint8 demonstrations stay lossless; `AgentCNN._encode_input` feeds it as `log1p(flow / FLOW_SCALE)`, a scalar plane alongside `FOOTPRINT`. **Adding it changed the encoder's `input_channels`, so checkpoints from before it do not load** — PPO needs an SFT base retrained on this tip, not `h76h80yb`.
