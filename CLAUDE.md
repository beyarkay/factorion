...
3. **RL finetuning** — PPO (`ppo.py`) refines the pretrained policy to push beyond what imitation achieves. Load an SFT checkpoint with `--start-from` (a local `.pt` path or a W&B run id, whose model artifact is downloaded automatically). Every episode builds from a **fully-blank** grid. The base must have been trained on the current architecture (any architecture change makes every older artifact unloadable, so mint a new production SFT run after one lands), and the bar PPO must clear is that base's own `val/thput` on the same lesson mix.
...
- **`ppo.py`**: `PpoArgs` (all PPO hyperparams + `start_from`, `critic_warmup`, `eval_every`; defined in `training_config.py`), `FactorioEnv` (`reset`/`step`; reward is the per-step **delta** of `score = thput_raw * cost_efficiency / max_throughput`, with `cost_efficiency = 1 / (1 + entity_cost_scale * entity_cost)` and entity cost = per-output recursively-expanded raw-item quantity + craft time, so an episode's return telescopes to its final-minus-reset score. Entity cost enters ONLY as a multiplicative discount on achieved throughput — at `thput_raw = 0` the score is 0 no matter what is on the grid, so building on a flowless grid is never charged and a zero-flow episode pays exactly zero reward at every step; there is no standalone cost penalty. The score is also uncapped above 1: out-producing the reference `max_throughput` keeps paying (#426); there is no "done" action — an episode runs to `max_steps`; `info['kind']` carries the lesson for per-lesson logging), `AgentCNN` (encoder + per-head outputs: tile/entity/direction/item/misc + `critic_head` + `thput_head`, a regression of the observed factory's items/s (`predicted_thput`; the head works in log1p space so belt and assembler rates sit an O(1) distance apart in the loss) that greedy rollouts stop on; stashes `_last_head_entropy` for the policy/\* metrics), `_resolve_start_from`/`_resolve_wandb_checkpoint` (checkpoint loading), `_run_signature` (run name), `_iteration_lrs` (actor + critic LRs follow independent WSD envelopes — `training_config.wsd_multiplier`, shared with `sft.build_lr_schedule`; knobs `lr_warmup_iters`/`lr_cooldown_frac`/`lr_min_ratio`, and `critic_lr`/`critic_lr_cooldown_frac`/`critic_lr_min_ratio` for the critic — no critic warmup ramp, the critic-warmup freeze is its warmup), `_build_eval_set`/`_run_greedy_eval` (the eval/ section).
- **`sft.py`**: `run_rollout_eval` returns `RolloutEval` with `overall` and per-`LessonKind` breakdowns; the rollout ends when the throughput head predicts at least `rollout_target_thput` (as it does in the web UI and the mod server), so the scored factory is the one the model thinks has reached the target. Checkpoint selection is on that `val/thput`. PPO reuses this for its `eval/` metrics (lazy import to avoid the ppo↔sft cycle). SFT trains the head with MSE in log1p(items/s) against each demo state's simulated `thput_raw` (`extract_expert_actions` labels every intermediate state); PPO does not train it.

## W&B dashboards

Runs are named by a hyperparameter signature, not a timestamp (`ppo.py:_run_signature`, `sft.py:_artifact_name`), e.g. `ppo-s11-lr3.369e-05-ent0.008034_0.0007372-kl0.02-cw9-fromu70e0sn2-c128-seed1`. `global_step` (env steps) is the PPO x-axis. PPO logs once per iteration into these sections (see `define_metric` block in `ppo.py`):

- **`eval/`** — periodic greedy held-out throughput at the state where the throughput head first predicts `rollout_target_thput` (`eval/thput`, `eval/{LESSON}/thput`), every `--eval-every` iters; directly overlay-able with the SFT baseline. **This is the headline progress signal**, and the sweep metric (`ci/sweep_ppo.yaml`). The same rollout also logs `eval/inserters_no_input` / `eval/inserters_no_output` (+ per-lesson and `eval/trial_*`; SFT: `val/…`): raw counts of inserters nothing feeds / that feed nothing across the eval set (`factorion.count_dangling_inserters`), which should be zero — the belt-stops-beside-the-inserter and inserter-feeds-inserter failures, visible without rendering factories — and `eval/rollout_thput_mae` (+ per-lesson; SFT: `val/rollout_thput_mae`), the head's mean |predicted − actual| items/s over every state the rollout visited, i.e. how much to trust where it stops.
- **`rollout/`** — on-policy sampled episode stats (`thput`, `thput_raw`, `reward`, `length`, `invalid_frac`, `num_entities`, `entity_efficiency`, `frac_reachable`, `entity_cost`, `cost_efficiency`) + per-lesson `rollout/{LESSON}/{thput,thput_raw,reward,length,entity_cost,cost_efficiency}` (raw items/s kept alongside the normalized throughput so lessons with very different ceilings stay comparable).
- **Lessons vs trials are pooled separately.** The `eval/thput` and `rollout/*` aggregates cover **lessons only**; trial episodes go to `eval/trial_thput` and the parallel `rollout/trial_*` block (same field names). This keeps the headline comparable to the SFT baseline — SFT never sees a trial, so mixing them in would move `eval/thput` for reasons unrelated to build skill. Per-lesson `{eval,rollout}/{TRIAL_*}/…` keys are already name-separated and are logged as usual, so a trial's own curve is always visible. **`eval/trial_thput` is the metric that actually matters** — it scores building a factory from nothing but the markers, which is the real problem; every lesson is a proxy for it. Expect it to stay low for a long time (#339): what there is comes from `TRIAL_RECIPE_TREE_DEPTH_1`; depths 2 and 3 are still exactly 0.
- **`policy/`** — acting-policy distribution: `entropy`, per-head `entropy_{tile,entity,direction,item,misc}`. With `--start-from`, also `kl_to_ref` + per-head `kl_to_ref_{head}`: closed-form drift from the frozen SFT reference (#237), logged whatever `divergence_penalty` is.
...
The metric comes from a greedy rollout that blanks the **whole** grid and rebuilds from empty (the honest "can it build from scratch" test). Per-factory throughput is `info['thput_normed']`, raw items/sec ÷ that factory's max **clipped to [0, 1]**, so a perfectly-rebuilt factory scores 1.0 regardless of belt speed. The PPO reward is not clipped, so a policy that out-produces the reference keeps earning return while this metric saturates at 1.0 — compare `rollout/{LESSON}/thput_raw` and episode length when the two disagree. The denominator is `Factory.max_throughput`, supplied by the generator: the solved world's simulated rate for lessons, an analytic single-assembler ceiling for trials (which have no reference solution to simulate). Defined in `sft.py:run_rollout_eval`:

- **`thput`** (`overall`) — build skill _respecting_ the throughput head: the rollout stops the first time `predicted_thput` reaches `rollout_target_thput` items/s (default 1.0) and scores the throughput at that state; if it never gets there, the rollout runs to max_steps and scores the final throughput. "How good is the factory at the moment the model thinks it has reached the target?" The target is a caller-side knob, so it can be raised to stop the model settling for a factory that merely flows.
...
**The RL goal is for PPO's `eval/thput` to exceed its own SFT base's `val/thput` on the same lesson mix.** The longer-run goal is `eval/trial_thput` climbing well above zero — nothing imitative can produce it.

## SFT → PPO handoff

`ppo.py --start-from <ckpt>` loads the full SFT `AgentCNN` state dict (encoder + every policy head + the throughput head). The **critic head is the only part SFT never trained**, so `--critic-warmup N` freezes the actor (encoder + policy heads) for N iterations and trains the value head alone against the fixed SFT features before joint PPO begins; LR envelopes + entropy schedule restart at the unfreeze. For finetuning an SFT policy, set `--ent-coef-start/--ent-coef-end 0` and a small `--learning-rate` (e.g. 5e-5); `--divergence-penalty β` additionally anchors the actor to the loaded checkpoint with a `β·KL(π_θ ‖ π_ref)` loss penalty against a frozen copy (#237), guarding SFT skills on lessons the current reward isn't exercising.
...
Judge a compare on the **difference curve**, never on endpoint values or the
report table alone. Pull both sides' full history of the metric from W&B,
interpolate onto a common x-axis (`train/samples_seen` for SFT, `global_step`
for PPO), take PR − main, smooth it (rolling window ≈ 10% of the run), and
report over the second half of the run: the mean smoothed difference, the
fraction of points where the raw difference is positive, and the difference's
own point-to-point std. A gap that keeps one sign at ≳ 90% of points and
clears the diff's std is real even at one seed per side; one that flips sign
or decays toward zero is not. Say whether the gap is growing, steady, or
closing. Endpoint deltas understate a steady gap by about half and invert small
per-lesson ones; seed std measured at another sample budget or architecture
does not transfer.
