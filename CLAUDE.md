...
- **`sft.py`**: `run_rollout_eval` returns `RolloutEval` with `overall` (throughput at the EOT fire) and per-`LessonKind` breakdowns; checkpoint is selected on `val/thput`. PPO reuses this for its `eval/` metrics (lazy import to avoid the ppo↔sft cycle).
...
- **`eval/`** — periodic greedy held-out throughput (`eval/thput`, `eval/{LESSON}/*`), every `--eval-every` iters; directly overlay-able with the SFT baseline. **This is the headline progress signal**, and the sweep metric (`ci/sweep_ppo.yaml`).
...
`thput` comes from a greedy rollout that blanks the **whole** grid and rebuilds from empty (the honest "can it build from scratch" test). Per-factory throughput is `info['thput_normed']` ∈ [0, 1] — raw items/sec ÷ that factory's max, so a perfectly-rebuilt factory scores 1.0 regardless of belt speed. Defined in `sft.py:run_rollout_eval`:

- **`thput`** (`overall`) — build skill *respecting* the EOT head: greedy-play each factory and take the throughput at the moment the EOT prob first crosses `rollout_eot_threshold` (0.5) — i.e. the factory at the moment the model decides it's done. If the EOT head never fires, the rollout runs to env-done (throughput 1.0 or max_steps) and takes the final throughput. The rollout never keeps placing past EOT.
