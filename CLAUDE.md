...
- **`rollout/`** — on-policy sampled episode stats (`thput`, `thput_raw`, `reward`, `length`, `eot_rate`, `invalid_frac`, `num_entities`, `entity_efficiency`, `frac_reachable`, `entity_cost`, `cost_efficiency`) + per-lesson `rollout/{LESSON}/{thput,reward,length,entity_cost,cost_efficiency}`. `thput_raw` is overall-only — it is the reward's input, but *within* a lesson the ceiling is fixed so raw is just an affine rescaling of `thput` (measured within-run correlation 1.000 on every fixed-ceiling lesson).
