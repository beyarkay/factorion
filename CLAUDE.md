...
- **Lesson mixture**: training samples lessons from `factorion.LESSON_WEIGHTS` (MEMORISE kinds 0.25, the rest 1.0, normalised) — SFT's pair stream (`_iter_demo_pairs`) and PPO's per-episode draw. The SFT val set and PPO's `_build_eval_set` stay balanced per kind so `val/thput`/`eval/thput` compare across mixtures (#408).
