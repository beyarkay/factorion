...
def request_to_world(req: dict) -> np.ndarray:
    ...
    `update_with_flow_and_thput` to get the policy's input."""
    size = req["grid_size"]
...
def _argmax_action(agent: AgentCNN, world_CWH: np.ndarray, device) -> dict:
    x = update_with_flow_and_thput(world_CWH)[0].unsqueeze(0).to(device)
    with torch.no_grad():
...
def run_inference(
    ...
    for step in range(max_steps):
        ...
        with torch.no_grad():
            x = update_with_flow_and_thput(world)[0].unsqueeze(0).to(device)
