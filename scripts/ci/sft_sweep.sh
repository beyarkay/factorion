#!/bin/bash
# sft_sweep.sh — Runs W&B sweep agents for SFT on a RunPod GPU pod.
#
# Builds the Rust extension, then launches multiple W&B sweep agents in
# parallel. Each agent pulls hyperparameters from the W&B Bayesian sweep
# controller, runs sft.py, and reports metrics back.
#
...
echo "============================================"
echo "  Factorion SFT W&B Sweep Agent"
echo "============================================"
...
# ── Override run_sft_sweep.sh for pod environment (no venv needed) ─
cat > run_sft_sweep.sh << 'EOF'
#!/bin/bash
python sft.py "$@"
EOF
chmod +x run_sft_sweep.sh
...
for i in $(seq 0 $((AGENTS_PER_POD - 1))); do
    ...
    if [ -f "$LOG" ]; then
        echo ""
        echo "--- SFT Agent ${AGENT_ID}.${i} log (last 20 lines) ---"
        tail -20 "$LOG"
    fi
done
