#!/usr/bin/env python3
"""Apply the best hyperparameters from a W&B SFT sweep to sft.py and open a PR.

Reads the best run from a completed sweep, updates the default values in the
SFTArgs dataclass in sft.py, and creates a pull request with the changes.
...
Usage:
    python scripts/ci/apply_sft_sweep_best.py \
...
def update_sft_defaults(sft_path: str, best_config: dict, sweep_url: str) -> list:
    """Update SFTArgs dataclass defaults in sft.py. Returns list of changed params."""
    ...
    for param_name, value in sorted(best_config.items()):
        ...
        if not match:
            print(f"  WARNING: could not find {param_name} in {sft_path}")
            continue
    ...
    sweep_comment = f"# Best SFT hyperparameters from W&B sweep: {sweep_url}"
    existing_pattern = r"^# Best SFT hyperparameters from W&B sweep: .+\n"
    if re.search(existing_pattern, content, re.MULTILINE):
        content = re.sub(existing_pattern, sweep_comment + "\n", content)
    else:
        content = content.replace("class SFTArgs:", sweep_comment + "\nclass SFTArgs:")
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply best SFT sweep hyperparameters to sft.py and open a PR"
    )
    parser.add_argument(
        "--sweep-path",
        required=True,
    ...
    parser.add_argument(
        "--output-pr-url",
        default="/tmp/sft_sweep_pr_url.txt",
    ...
    run(["git", "add", args.sft_file])

    commit_lines = [
        f"Update SFT hyperparameters from sweep {sweep_id}",
        "",
        "Applied best hyperparameters from W&B SFT sweep:",
        f"  {sweep_url}",
        "",
    ...
    pr_body_lines = [
        "## Apply best SFT hyperparameters from sweep",
        "",
        f"Automatically generated from W&B SFT sweep [{sweep_id}]({sweep_url}).",
        "",
    ...
    if args.pr_number:
        pr_body_lines.append(f"SFT sweep triggered from PR #{args.pr_number}.")
    pr_body_lines.append(f"\n[View sweep on W&B]({sweep_url})")
    pr_body = "\n".join(pr_body_lines)

    pr_title = f"Update SFT hyperparameters from sweep {sweep_id}"
