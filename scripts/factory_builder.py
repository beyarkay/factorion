...
def _load_checkpoint(path: str) -> None:
    ...
    # critic/throughput-head filtering below silently misses every key.
...
def _get_agent(size: int) -> AgentCNN:
    ...
    # a size match. A checkpoint without thput_head keys → load_state_dict
    ...
    ignorable = {
        ...
        "thput_head.weight", "thput_head.bias",
        "attn.pos_embed",
    }
...
def _predict_action(grid: list[list[dict]]) -> dict:
    """Return only the greedy next placement, plus the predicted throughput.
    ...
    none), and skips `predicted_thput`, without which the loop cannot stop where
    a rollout would.
    """
    world_WHC = build_world(grid)
...
def _rollout_result(
    index: int, env: FactorioEnv, seed: int, reached_target: bool
) -> dict:
    return {
        "type": "result",
        "index": index,
        ...
        "size": env.size,
        "steps": int(env.steps),
        "stopped_by": "target" if reached_target else "max_steps",
        "thput_normed": float(env._thput_normed),
        "thput_raw": float(env._thput_raw),
        "max_throughput": float(env._max_throughput),
        "num_placed_entities": int(env._num_placed_entities),
        "frac_reachable": float(env._frac_reachable),
...
def _batch_rollout(
    ...
    rollout ends where the throughput head first predicts ``target_thput``,
    since the whole point is to see the factory the model considers finished.
    """
    agent = _get_agent(size)
        ...
        with torch.no_grad():
            while any(active):
                ...
                for i, env in enumerate(envs):
                    if not active[i]:
                        continue
                    if float(predicted[i]) >= target_thput:
                        active[i] = False
                        yield _rollout_result(base + i, env, used_seeds[i], True)
                        continue
                    action = {
                    ...
                    if terminated or truncated:
                        active[i] = False
                        yield _rollout_result(base + i, env, used_seeds[i], False)
...
def _batch_rollout_request(payload: dict) -> Iterator[dict]:
    ...
    try:
        ...
        yield from _batch_rollout(
            ...
            target_thput=float(payload.get("target_thput", THPUT_STOP_TARGET)),
        )
    except Exception as e:
        traceback.print_exc()
...
<div id="tab-scan" hidden>
  <div class="controls action-row">
    ...
    <button id="scan-run" title="Blank each seed's factory, let the model rebuild it until it predicts the target throughput, and add every final factory to the gallery">
      Run scan
    </button>
...
const THPUT_STOP_TARGET = {THPUT_STOP_TARGET};
...
function thputStop(pred) {{
  return !!pred && pred.predicted_thput >= THPUT_STOP_TARGET;
}}
...
function renderGrid() {{
  ...
  // ghost lookup is O(1).
  // Once the model predicts the target throughput a rollout would have
  // stopped — the candidate ghosts would just be misleading hallucinations
  // of a forced placement, so suppress them.
  const candByXY = {{}};
  if (prediction && prediction.candidates && !thputStop(prediction)) {{
  ...
  for (let y = 0; y < SIZE; y++) {{
    ...
    for (let x = 0; x < SIZE; x++) {{
      ...
      // ghost overlays: if the model would have stopped, don't visually
      // nominate a "next placement" tile.
      if (
        prediction && prediction.x === x && prediction.y === y
        && !thputStop(prediction)
...
async function computePrediction() {{
  ...
  try {{
    ...
    showThput(data);
    if (info) {{
      info.textContent = thputStop(data)
        ? thputStopMessage(data) + ' — no placement offered'
...
function applyPrediction() {{
  ...
  if (thputStop(prediction)) {{
    const info = document.getElementById('model-info');
    if (info) info.textContent = thputStopMessage(prediction) + ' — nothing applied';
    return;
  }}
...
// ── Scan tab ────────────────────────────────────────────────────────────
// A scan blanks N factories, lets the model rebuild each one until it predicts
// the target throughput, and lays the finished factories out side by side.
let scanResults = [];
let scanAbort = null;
let scanIndexBase = 0;
...
function scanCard(r, showRef) {{
  ...
  const stop = r.stopped_by === 'target'
    ? 'stopped at ' + r.steps
...
// A factory counts as complete at thput 1.0.
...
function scanSummary(status) {{
  ...
  const stopped = scanResults.filter(r => r.stopped_by === 'target').length;
  el.textContent =
    `${{n}} done · mean thput ${{mean.toFixed(3)}} · ${{zeros}} at zero · ` +
    `${{perfect}} perfect · target reached ${{stopped}}/${{n}}` +
    (status ? '  ·  ' + status : '');
}}
...
function renderScanStats() {{
  ...
  const rows = [...byKind.entries()].map(([kind, rs]) => {{
    ...
    return {{
      ...
      stopped: rs.filter(r => r.stopped_by === 'target').length,
    }};
  }});
  ...
  const body = rows.map(row =>
    ...
    `<td>${{frac(row.stopped, row.n)}}</td></tr>`
  ).join('');
...
async function runScan() {{
  ...
  const body = {{
    ...
    target_thput: parseFloat(document.getElementById('scan-target').value),
