...
/// Mirror a square world left-right: cell `(x, y)` → `(s-1-x, y)` with
/// East/West facings swapped. Together with [`rotate_world_cw`] this reaches
/// all 8 grid symmetries of a canonical layout.
fn flip_world_x(world: &World) -> World {
    let s = world.width();
    debug_assert_eq!(s, world.height(), "flip assumes a square grid");
    let mut out = World::empty(s, s);
    for x in 0..s {
        for y in 0..s {
            if world.entity_at(x, y).is_none() {
            ...
            let nx = s - 1 - x;
            out.set(nx, y, Channel::Entities, world.get(x, y, Channel::Entities));
            let ndir = match world.direction_at(x, y) {
                Direction::East => Direction::West,
                Direction::West => Direction::East,
                d => d,
            };
            out.set(nx, y, Channel::Direction, ndir as i64);
            out.set(nx, y, Channel::Items, world.get(x, y, Channel::Items));
            out.set(nx, y, Channel::Misc, world.get(x, y, Channel::Misc));
        }
    }
    out
}
...
/// MEMORISE lessons (whose source/sink hug the assembler), the source and
/// sink sit at semi-arbitrary free cells on OPPOSITE sides of the row — the
/// row splits the world in two and the source lands in the input-lane half,
...
/// limited — giving the critic both good and bad layouts to rank, which is
/// why `max_throughput` is the analytic [`assembler_row_ceiling`] of a full
/// row rather than this one sample from that spread. Dead or orphan-tile
...
fn build_factory_1_ingredient(
    ...
    while count > 0 {
        ...
        // Assembler row: as many 3×3 machines as fit with `gap` empty columns
        // between neighbours (3n + gap(n-1) <= s).
...
/// underground end where ingredient B is at surface level, or a plain-belt
...
fn build_factory_2_ingredients(
    ...
    // helper), ingredient-B lane (near belt / weave column), input inserters,
    // 3 assembler columns, output inserters, output lane.
    if s < 8 {
        return None;
    }
    ...
    // The output-lane direction is drawn ONCE, outside the rejection loop —
    // its distribution is the lesson spec, and rerolling it per attempt would
    // skew the realized mix toward whichever rejects least.
    let north_sink = rng.choice_index(2) == 1;
    let head = i64::from(north_sink);
    // The column is packed to capacity (one row below for the source band,
    // one above for a north exit), so every reference strictly needs all of
    // its machines (#410).
    let n_asm = (s - 1 - head) / 3;
    ...
    while count > 0 {
        ...
        if feed == Feed::Weave {
            ...
            loop {
                ...
                // needs run(2+) + up = 3 rows); inside it, pair or stop 50/50.
                if remaining >= 3 && ((tiles.len() as i64) < top_lo || rng.choice_index(2) == 0) {
            ...
            // The two dead ends — the topmost mouth (a final `u`, or an
            // unpaired stopper `d`) and the topmost run tile — strand items
            ...
            let (Some(dead_run), Some(dead_mouth)) = (
                (0..len)
                    .rev()
                    .find(|&t| tiles[t as usize] == WeaveTile::Run),
                (0..len).rev().find(|&t| is_mouth(t)),
            ) else {
                continue;
            };
            let window_of = |t: i64| {
            ...
            for j in 0..n_asm {
                ...
                let tm = if j == jm {
                    ...
                    mouths[rng.choice_index(mouths.len())]
                };
                let tr = if j == jr {
                    dead_run
                } else {
                    runs[rng.choice_index(runs.len())]
                };
                for t in [tm, tr] {
                    inserters.push(((col_in, src_row - 1 - t), Item::Inserter));
                }
            }
            if !ok {
                continue;
            ...
            // Emit the column. A run's top tile turns off west — its `b<`
            // hands A to the helper, or dead-ends for the topmost run —
            // unless the run is a single tile (a west-facing tile would
            // refuse the helper's head-on feed).
            for t in 0..len {
                let row = src_row - 1 - t;
                match tiles[t as usize] {
                    ...
                    WeaveTile::Run => {
                        ...
                        let dir = if top && !bottom {
                            Direction::West
                        } else {
                            Direction::North
                        };
                        belts.push((col_b, row, dir, Misc::None));
                    }
                }
            }
            ...
            let mut t = 1;
            while t < len {
                if tiles[t as usize] == WeaveTile::Run {
                    let bot_row = src_row - 1 - t;
                    belts.push((col_a, bot_row, Direction::East, Misc::None));
                    for r in bot_row + 1..=lower_end {
                        belts.push((col_a, r, Direction::North, Misc::None));
                    }
                    while t < len && tiles[t as usize] == WeaveTile::Run {
                        t += 1;
                    }
                    lower_end = src_row - t;
                } else {
...
/// inserter slots one face of a 3×3 machine offers in these layouts. Being
/// `machines` single-machine ceilings, it scores a one-machine build at
/// `1 / machines`.
///
...
/// caps the ratio at `feed / Σ consumes`), and the output inserters drain at
/// most `feed`.
...
mod tests {
    ...
    fn test_factory_1_ingredient_needs_every_assembler() {
        // Every reference build out-produces the best a SINGLE machine in the
        // row layout can do — three plain inserters in, three out — so a
        // one-assembler build can never match it (#410).
        let mut checked = 0;
        for seed in 0..50u64 {
            let Some(f) = build_factory(
                11,
            ...
            let single = assembler_row_ceiling(&recipe, 1);
            let reference = tp_unreachable(&f.world).0;
            assert!(
                reference > single + 1e-9,
                "seed={seed}: {recipe_item:?} reference {reference} <= \
                 single-assembler bound {single}"
            );
            checked += 1;
        }
    ...
    fn test_factory_lesson_max_throughput_is_a_ceiling() {
        // `max_throughput` is the analytic ceiling for the machines the grid
        // fits, not the sampled build's own rate (#426). The reference still
        // draws 2-3 inserters a side, so its rate varies and a
        // better-inserted build beats it — but nothing beats the ceiling.
        // Being `machines` single-machine ceilings, it also puts a
        // one-machine build at exactly `1 / machines`. It stays attainable,
        // though — some build gets most of the way there.
        for (size, machines) in [(11usize, 3i64), (14, 4)] {
            for kind in [
                ...
                for seed in 0..120u64 {
                    ...
                    assert_eq!(
                        f.max_throughput,
                        machines as f64 * assembler_row_ceiling(&recipe, 1),
    ...
    fn test_factory_2_ingredients_both_sources_necessary() {
        ...
        assert!(checked > 0, "no factories built");
    }
