...
from ci.config import (
    ...
    CLAUDE_MAX_NUM_SAMPLES,
    CLAUDE_MAX_SEEDS,
    CLAUDE_MAX_SWEEP_RUN_CAP,
...
# The attribution footer Claude Code signs its comments with — the only signal
# separating an agent-authored command from one a person typed. Both wordings
# in circulation ("Generated by/with [Claude Code](...)") count.
_CLAUDE_FOOTER_RE = re.compile(r"generated (?:by|with)\s*\[?claude code", re.I)


# "b" is not SI, but nobody writing 2B timesteps in a PR comment means bytes.
_SI_SCALE = {"": 1, "k": 10**3, "m": 10**6, "g": 10**9, "b": 10**9}


def si_int(text: str) -> int:
    """A count written as 5M / 4.5M / 500k / 40000000. Decimal, not float, so
    a scaled fraction is exact (1.005M is 1005000, not 1004999.99...)."""
    match = re.fullmatch(r"(\d+(?:\.\d+)?)\s*([kmgb]?)", text.strip(), re.I)
    if not match:
        raise argparse.ArgumentTypeError(f"{text!r} is not a count like 5M, 500k or 40000000")
    scaled = Decimal(match.group(1)) * _SI_SCALE[match.group(2).lower()]
    if scaled != scaled.to_integral_value():
        raise argparse.ArgumentTypeError(f"{text!r} does not scale to a whole number")
    return int(scaled)


def _cap(ctx, value: Optional[int], limit: int, unset: Optional[int] = None):
    """Cap one cost knob when the /ci comment was written by an agent: nobody
    reviews it before the pod boots. `unset` is what a missing flag resolves to
    downstream, passed only when that default itself blows the limit."""
    if not ctx["claude"]:
        return value
    if value is None:
        return None if unset is None else min(limit, unset)
...
def cmd_sft(args, ctx) -> None:
    # SftArgs' 45M default is ~20h of GPU, so a bare `/ci sft` needs the cap
    # as much as an explicit --num-samples does.
    args.num_samples = _cap(
        ctx, args.num_samples, CLAUDE_MAX_NUM_SAMPLES, SftArgs.num_samples
    )
    job = SftJob(
...
def cmd_ppo(args, ctx) -> None:
    args.total_timesteps = _cap(ctx, args.total_timesteps, CLAUDE_MAX_TOTAL_TIMESTEPS)
    job = PpoJob(
        sha=ctx["sha"],
...
def cmd_compare(args, ctx) -> None:
    args.seeds = _cap(ctx, args.seeds, CLAUDE_MAX_SEEDS)
    args.num_samples = _cap(ctx, args.num_samples, CLAUDE_MAX_NUM_SAMPLES)
    args.total_timesteps = _cap(ctx, args.total_timesteps, CLAUDE_MAX_TOTAL_TIMESTEPS)
    nonce = compare_nonce()
...
def cmd_sweep(args, ctx) -> None:
    algo = args.algo
    sweep_config = read_sweep_config(algo, ctx["sha"])
    # Refused rather than capped: the run count lives in the yaml, not in a
    # flag the dispatcher can rewrite. No run_cap means unbounded.
    run_cap = sweep_config.get("run_cap")
    if ctx["claude"] and (run_cap is None or run_cap > CLAUDE_MAX_SWEEP_RUN_CAP):
        _post(
            ctx,
            f"## &#x274C; sweep refused\n\n"
            f"`ci/sweep_{algo}.yaml` has run_cap {run_cap or 'unset'}; a `/ci` "
            f"comment carrying the Claude Code footer may launch at most "
            f"{CLAUDE_MAX_SWEEP_RUN_CAP} runs.\n\n"
            f"Claude: lower `run_cap` to {CLAUDE_MAX_SWEEP_RUN_CAP} or fewer and "
            "post the command again. Do **not** work around this guard — not by "
            "stripping the footer, not by launching pods another way, not by "
            "editing the limit. If the sweep genuinely needs to be bigger, say so "
            "and ask the user to post the comment themselves.",
        )
        raise SystemExit(1)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--total-timesteps", type=si_int, default=None)
    common(sp)
    ...
    sp.add_argument("--num-samples", type=si_int, default=COMPARE_NUM_SAMPLES_DEFAULT)
...
def main() -> None:
    ...
    ctx["claude"] = bool(_CLAUDE_FOOTER_RE.search(body))
