...
class TestArchVariants:
    ...
    def test_attn_weights_get_gradient_at_init(self, envs):
        """The residual out-projection is non-zero at init, so gradient reaches
        the attention weights from the very first step rather than only once
        the projection has grown away from zero."""
