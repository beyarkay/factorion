...
class TestReporter:
    ...
    def test_run_summary_contains_marker_and_headline(self):
        md = run_summary_markdown(
            ...
            kind="sft",
            sha7=SHA[:7],
            summary_flat={"greedy/thput": 0.31, "val/acc": 0.9, "obscure/x": 1.0},
        )
        assert "<!-- factorion-ci-run:abc123 -->" in md
        before_details, details = md.split("<details>", 1)
        assert "greedy/thput" in before_details
...
class TestSelectHeadline:
    def test_sft_patterns_cover_every_lesson_and_head(self):
        names = [
            "greedy/thput",
            "greedy/MOVE_ONE_ITEM/thput",
            "greedy/SPLITTER_SPLIT/thput",
            "greedy/SOME_FUTURE_LESSON_9/thput",  # lessons matched, not hardcoded
        ...
        assert got == [
            "greedy/thput",
            "greedy/MOVE_ONE_ITEM/thput",
            "greedy/SOME_FUTURE_LESSON_9/thput",
            "greedy/SPLITTER_SPLIT/thput",
            "val/acc",
            "val/eot_acc",
            "val/tile_acc",
