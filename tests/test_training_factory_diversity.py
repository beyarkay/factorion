...
class TestPPOFactoryDiversity:
    ...
    def test_sync_vector_env_does_not_replay_factories(self):
        ...
        # several step-budget-truncated episodes and assert every env, every episode,
        ...
        for _ in range(6):
            envs.step(placement_action())
            envs.step(placement_action())
            envs.step(placement_action())  # max_steps -> truncate
            envs.step(placement_action())  # NEXT_STEP autoreset fires
