...
each entity. These cover the whole path — engine → `observe` → env → network.
"""
...
class TestObserve:
    def test_appends_flow_after_the_world_channels(self):
        ...
        world = belt_line()
        obs, thput, unreachable = observe(world)
    ...
    def test_flow_follows_the_line_and_stops_at_empty_tiles(self):
        ...
        flow = observe(belt_line())[0][CH_FLOW] / FLOW_SCALE
    ...
    def test_channel_is_integer_valued_so_uint8_storage_is_lossless(self):
        ...
        obs = observe(belt_line(middle="inserter"))[0]
    ...
    def test_keeps_the_world_dtype(self):
        ...
        world = belt_line()
        assert observe(world)[0].dtype == world.dtype
        assert observe(world.float())[0].dtype == torch.float32
    ...
    def test_flow_is_zero_on_a_belt_loop(self):
        ...
        obs, thput, unreachable = observe(world.permute(2, 0, 1).contiguous())
        assert thput == 0.0
        assert unreachable == 0
